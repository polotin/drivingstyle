...
function fill_table(json_str, json_str_config) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        html += "<tr>" +
            ...
            "<td class=\"center\"><button class='fa fa-video-camera video_btn' onclick=\"playMyVideo('" + data[i]['csv_file_name'] + "','" + data[i]['driver_id'] + "','" + data[i]['trip_id'] + "','" + start_time + "','" + stop_time + "'); this.style ='background-color:#DBDBDB;'\">&nbsp;Video</button></td>" +
            "</tr>";
    }
    var table_area = document.getElementById("page-wrapper1");
    var table_body = document.getElementById("table_body");
