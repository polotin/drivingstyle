    ...
    for (var i = 0; i < length; i++) {
        ...
        switch (data[i]['type']) {
            ...
            case 'car_following':
                ...
                break;
            case 'lane_change':
                start_time = start_time * 10 - parseFloat(config_data['backward_lane_change']);
                stop_time = stop_time * 10 + parseFloat(config_data['forward_lane_change']);
                break;
            default:
                start_time -= 5;
                stop_time += 5;
...
function showEventChart(driver_id, trip_id, event_id, type, cur_time) {
    ...
    if (type == "stop" || type == "go") {
        ...
        var chart_link = "eventChart.php?file_dir=" + csv_file_dir + "&driver_id=" + driver_id + "&trip_id=" + trip_id + "&event_id=" + event_id + "&event_type=" + type;
        window.open(chart_link, "newwindow", "height=800, width=1050, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
    }else if(type.indexOf("hard_swerve") == 0){
        if(document.cookie.split(";")[0].split("=")[1]=="true"){
            csv_file_name = driver_id + "_" + "_" + "hard_swerve" + cur_time + ".csv";
        }else {
        ...
        var chart_link = "eventChart.php?file_dir=" + csv_file_dir + "&driver_id=" + driver_id + "&trip_id=" + trip_id + "&event_id=" + event_id + "&event_type=" + type;
        window.open(chart_link, "newwindow", "height=800, width=1050, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
    }else if(type == "car_following"){
        if(document.cookie.split(";")[0].split("=")[1]=="true"){
            csv_file_name = driver_id + "_" + "_" + "car_following" + cur_time + ".csv";
        }else {
